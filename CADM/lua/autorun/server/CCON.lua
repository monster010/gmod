...
hook.Add('player_connect','CCON',function(data)
	...
	http.Fetch('http://api.sypexgeo.net/json/'..ip,function(b)
		...
		if !ip or ip == 'loopback' or ip == 'none' then
			ChatNet(CRANK.GetRankTable(data.networkid).color or Color(255,255,0),data.name,Color(150,150,150),' (',Color(0,0,0),data.networkid,Color(150,150,150),') is connecting')
		...
		ChatNet(CRANK.GetRankTable(data.networkid).color or Color(255,255,0),data.name,Color(150,150,150),' (',Color(0,0,0),data.networkid,Color(150,150,150),') is connecting',' ('..Profitrole(see.city.name_ru,see.region.name_ru,see.country.name_ru)..', ',Color(150,150,150),')')
		MsgC(Color(220,0,255),'[CCON]: ',Color(255,150,0),'Connecting '..data.name..' ['..ip..']\n')
	end)
end)
...
hook.Add('player_disconnect','CCON',function(data)
	...
	Send(CCON)
	ChatNet(CRANK.GetRankTable(data.networkid).color or Color(255,255,0),data.name,Color(150,150,150),' (',Color(0,0,0),data.networkid,Color(150,150,150),') disconnected')
